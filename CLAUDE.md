...
The repo is a monorepo. `packages/contracts`, `packages/types`, `packages/sdk`, `packages/config`, `apps/execution-node`, `apps/indexer`, and `apps/api` are implemented (see below). `apps/frontend` doesn't exist on disk at all yet — it's named in the README's repository-structure diagram as intended future layout, not an actual `.gitkeep` stub. Don't assume it exists; check before referencing a path under it.
